<template>
  <div class="w-full h-fit min-h-[calc(100dvh-4.5rem)] relative py-5 rounded-xl flex flex-col items-start justify-start  overflow-hidden bg-c-white">
    <div class="start flex flex-col h-full w-full relative">
      <h2 class="px-5 text-c-dark text-[3rem] font-title font-bold">
        <ScrambledText :text="$t('pageTitle.stack')"/>
      </h2>
      <div class="flex flex-col ml-auto mt-auto w-2/3">
        <div
            :class="['appear', { show: isVisible }]"
            v-for="(item, index) in stackList"
            :key="item.name"
            class="flex w-ful py-1.5 l gap-5 px-5 relative border-b border-dotted text-c-dark  border-b-transparent hover:border-b-c-dark hover:bg-neutral-800 hover:text-c-white"
        ...
        <span class="mt-1 text-[0.45rem]">
          ...
          <span class= "font-black text-[1rem]">{{ item.name }}</span>
          <span class= "text-right ml-auto">Lorem ipsum dolo Lorem ipsum dolores Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut.</span>
        </div>
      </div>
      <transition name="fade">
        <Icon v-if="currentIcon" class="text-c-dark absolute bottom-3 left-5" :name="currentIcon" size="5rem"/>
      </transition>
    </div>
  </div>
</template>
...
.appear {
  max-height: 0;
  overflow: hidden;
  transition: max-height 6s ease, padding 0.3s ease;
  padding-top: 0;
  padding-bottom: 0;
}
...
.fade-enter-active,
.fade-leave-active {
  transition: opacity 0.1s ease, filter 0.5s ease;
