<template>
  <div
    class="relative flex h-fit min-h-[calc(100dvh-4.5rem)] w-full flex-col items-start justify-start overflow-hidden rounded-xl bg-c-white px-5 py-5"
  >
    <div class="start relative flex h-full w-full flex-col">
      ...
      <div class="grid h-full w-full grid-cols-7 grid-rows-1 gap-x-3 gap-y-10">
        <div
          v-for="(project, index) in projectsList"
          :key="index"
          class="flex w-full flex-col items-start justify-start gap-2"
        >
          <span class="font-main text-[0.65rem] font-bold tracking-wider text-c-dark uppercase">
            {{ `[ ${project.name} ]` }}
          </span>
          ...
          <span class="text-c-dark">
            {{ project.description }}
          </span>
          <span class="text-[0.65rem] font-bold text-c-dark">
            {{ project.stack }}
          </span>
        </div>
      </div>
...
const projectsList = [
  ...
  },
  {
    ...
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent non ligula condimentum,' +
      ' sagittis odio vitae, dapibus nisi. Nulla et malesuada diam. Fusce vestibulum ipsum a.',
    stack: 'Nuxt 3, TypeScript, i18n, Tailwind, Pixi.js, Three.js',
    imgList: ['/images/projects/pres1.png', '/images/projects/pres2.png'],
  },
];
